    ...
    if (length(which(sub_groups$start == i)) == 1) {
      ...
      if (length(bumphunter_match_index) >= 1){
        ...
        for (bump_index in 1:length(bumphunter_match_index)){
          ...
          pdf(paste0("/home/jyu/scratch/Marie_Hudson_Scleroderma_WGBS/final_figures/SMAD3_Overlapping_Bumphunter_Beta_Values:Chr_", chr, "_Gene_", gene, "_DMR_", j, ".pdf"), width = 10, height = 7)
          #margins
          par(mar=c(6,6,6,14))
